						...
						<view class="swiper-item">
							<scroll-view class="scroll-view-item" scroll-y="true">
								<view class="address_item" v-for="(item,index) in cityAreaArray[swiper_index]" :key="index" @click="address_item_click(swiper_index,index)">
									<image v-if="selectIndexArr[swiper_index] === index" class="address_item_icon" src="../../static/json/yixuan-selectAddress/gou.png" mode=""></image>
									{{item.name}}
								</view>
							</scroll-view>
						</view>
...
<script>
	import cityData from '../../static/json/yixuan-selectAddress/city.json'
	export default {
		data() {
			return {
				isShow: false,
